...
export class GameContext {
  ...
  onFlip(event) {
    ...
    const name = player.hand[event.hand_index]["name"];
    handSprite.texture = this.textures[name];
  ...
  onTakeDeck(event) {
    ...
    if (player.id === this.game.playerId) {
      makePlayable(this.sprites.held, () => this.onHeldClick())//this.onHeldClick.bind(this));
  ...
  addHand(player) {
    ...
    sprites.forEach((sprite, i) => {
      if (player.id == this.game.playerId && this.isPlayable(`hand_${i}`)) {
        makePlayable(sprite, () => this.onHandClick(player.id, i));
      }
